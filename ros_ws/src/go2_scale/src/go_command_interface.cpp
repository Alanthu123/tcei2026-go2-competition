...
{
    const std::string pattern = std::string(R"([\"'])") + key + R"([\"']\s*:\s*(-?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?))";
    std::smatch match;
...
private:
    void llmNavGoalCallback(const std_msgs::String::ConstPtr& msg)
    {
        ...
        if (!extractJsonNumber(json_str, "x", x) ||
            !extractJsonNumber(json_str, "y", y) ||
            !extractJsonNumber(json_str, "yaw", yaw))
        {
            ROS_ERROR("解析导航目标失败，缺少 x/y/yaw 字段: %s", json_str.c_str());
            return;
        }
